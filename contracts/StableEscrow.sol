...
contract StableEscrow {
    bytes32 constant DAI_BEACON_ID =
        0x72c512f825577f581e7100dba06031dc34c748e948bb060af51ba76112a9abc0; // this is the same across all chains for Amberdata DAI/USD beacon
    ...
    constructor(
        ...
        address _stablecoin,
        address _beaconRrp,
        address _selfServeBeaconWhitelister
    ) {
        ...
        iBeaconWhitelister = ISelfServeRrpBeaconServerWhitelister(
            _selfServeBeaconWhitelister
        );
        iBeaconWhitelister.whitelistReader(DAI_BEACON_ID, escrowAddress);
    ...
    function closeDeal() external returns (bool, int224) {
        ...
        (int224 _value, uint32 _timestamp) = ibeacon.readBeacon(DAI_BEACON_ID); // hardcoded DAI for testing, could have selection between DAI/USDC/USDT Beacon IDs in future versions
        if (
            _value < 970000 || _value > 1030000
        ) /* if the DAI price is +- 3% off $1 peg */
        {
