...
contract AirnodeEthEscrow is RrpRequester {
  ...
  constructor(string memory _description, uint256 _deposit, uint256 _secsUntilExpiry, address _seller, address _airnodeRrp) RrpRequester(_airnodeRrp) {
      if (_seller == msg.sender) revert BuyerAddress();
      buyer = payable(msg.sender);
      deposit = _deposit;
      escrowAddress = payable(address(this));
  ...
  /// @notice buyer deposits in escrowAddress, requires exact wei deposit amount
  function depositInEscrow() external nonReentrant returns(bool, uint256) {
      if (msg.sender != buyer) revert OnlyBuyer();
      if (msg.value != deposit) revert DepositAmount(); // requires buyer deposit exactly the deposit amount in wei
      (bool sent,) = escrowAddress.call{ value: deposit }("");
      if (!sent) revert PaymentFailed();
      return (true, escrowAddress.balance);
  ...
  function _closeDeal(int256 _decodedData) private nonReentrant returns(bool) {
